...
public class Shield extends PowerUP {
    private Image img;
    private World world;
    public Shield(int width, int height, Image img, JFrame frame, World world){
    ...
    public void intersection(ArrayList<PowerUP> powerUPSToRemove){
        ...
        if (powerup.intersects(player)) {
            ...
            if(world.player.getHealth() + 300 >= 1000) {
                world.player.setHealth(1000);
            } else {
                world.player.setHealth(world.player.getHealth() + 300);
            }
            new Thread(new SoundEffect("sound/heal.wav")).start();
        }
    }
}
